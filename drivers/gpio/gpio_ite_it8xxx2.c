...
struct gpio_ite_cfg {
	...
	char *label;		/* label is set by device tree */
	int gpios_pin;		/* pin */
	int gpio_irq;		/* gpio's irq */
	int flag;		/* gpio's setting */
	bool port_is_output;	/* to record output or input */
};
...
static inline uint8_t get_port(const struct gpio_ite_cfg *config)
{
	...
	return (regv & BIT(config->gpios_pin));
}
	...
	gpio_pin_t pin, enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	g = gpio_config->gpio_irq / NUM_IO_MAX;
	i = gpio_config->gpio_irq % NUM_IO_MAX;
	trig_mode = reg_ielmr[g];
	hl_trig = reg_ipolr[g];

	if (mode & GPIO_INT_MODE_DISABLED) {
		/* Disables interrupt for a pin. */
		ite_intc_irq_disable(gpio_config->gpio_irq);
		return ret;
	...
	for (gpio_index = 0; gpio_index < CURR_SUPP_GPIO_SET; gpio_index++) {
		if (gpio_config->reg_addr ==
			...
			wui_local = (struct gpio_ite_wui *)
				&(gpiox_reg[gpio_index].wui
				[gpio_config->gpios_pin]);
	...
	ret = irq_connect_dynamic(
		gpio_config->gpio_irq, 0, gpio_ite_isr, dev, 0);
	ite_intc_irq_enable(gpio_config->gpio_irq);
	return ret;
}
...
static int gpio_ite_init(const struct device *dev)
{
	...
	if (gpio_config->reg_addr == GPIO_GPDRM) {
		gpcr_offset = GPIO_DIR_OUTPUT;
	}
	...
	if (gpio_config->port_is_output) {
		ite_write(gpcr_reg_addr + gpio_config->gpios_pin,
				1, GPIO_DIR_OUTPUT);
		if (gpio_config->flag & GPIO_OUTPUT_INIT_HIGH) {
			set_bit(gpio_config, gpio_config->gpios_pin,
					GPIO_HIGH);
		} else {
			set_bit(gpio_config, gpio_config->gpios_pin,
					GPIO_LOW);
		...
		ite_write(gpcr_reg_addr + gpio_config->gpios_pin, 1,
				GPIO_DIR_INPUT);
		regv = ite_read(gpio_config->reg_addr, gpio_config->reg_size);
	}
	if (gpio_config->flag & GPIO_INT_ENABLE) {
...
#define GPIO_ITE_DEV_CFG_DATA(n, idx, is_out) \
static struct gpio_ite_data gpio_ite_data_##n##_##idx; \
static const struct gpio_ite_cfg gpio_ite_cfg_##n##_##idx = { \
	.reg_addr = DT_REG_ADDR_BY_IDX(DT_PHANDLE_BY_IDX\
			(DT_NODELABEL(n), gpios, idx), idx),\
	.label = DT_LABEL(DT_PHANDLE_BY_IDX(DT_NODELABEL(n), gpios, idx)), \
	.gpios_pin = DT_PHA_BY_IDX(DT_NODELABEL(n), gpios, idx, pin), \
	.port_pin_mask = BIT(DT_PHA_BY_IDX(DT_NODELABEL(n), gpios, idx, pin)),\
	.port_is_output = is_out, \
	}; \
...
GPIO_ITE_DEV_CFG_DATA(room_led, 0, 1);
#endif /* gpio_leds */
#if DT_NODE_HAS_STATUS(DT_INST(0, gpio_keys), okay)
GPIO_ITE_DEV_CFG_DATA(room_sw, 0, 0);
#endif /* gpio_keys */

/* test_gpio_basic_api */
#if DT_NODE_HAS_STATUS(DT_INST(0, test_gpio_basic_api), okay)
static struct gpio_ite_data gpio_ite_data_2;
static struct gpio_ite_data gpio_ite_data_3;
static const struct gpio_ite_cfg gpio_ite_cfg_2 = {
	.reg_addr = DT_REG_ADDR_BY_IDX(DT_PHANDLE_BY_IDX(
		DT_INST(0, test_gpio_basic_api), in_gpios, 0), 0),
	.gpios_pin = DT_PHA_BY_IDX(
		DT_INST(0, test_gpio_basic_api), in_gpios, 0, pin),
	.flag = GPIO_INPUT,
	.gpio_irq = 110,
};
static const struct gpio_ite_cfg gpio_ite_cfg_3 = {
	.reg_addr = DT_REG_ADDR_BY_IDX(DT_PHANDLE_BY_IDX(
		DT_INST(0, test_gpio_basic_api), out_gpios, 0), 0),
	.gpios_pin = DT_PHA_BY_IDX(
		DT_INST(0, test_gpio_basic_api), out_gpios, 0, pin),
	.flag = GPIO_OUTPUT_HIGH,
};
DEVICE_AND_API_INIT(gpio_it8xxx2_gpiof_2, DT_GPIO_LABEL(
			    DT_INST(0, test_gpio_basic_api), in_gpios),
		    gpio_ite_init,
		    &gpio_ite_data_2,
		    &gpio_ite_cfg_2,
		    POST_KERNEL,
		    CONFIG_KERNEL_INIT_PRIORITY_DEFAULT,
		    &gpio_ite_driver_api);
DEVICE_AND_API_INIT(gpio_it8xxx2_gpiof_3, DT_GPIO_LABEL(
			    DT_INST(0, test_gpio_basic_api), out_gpios),
		    gpio_ite_init,
		    &gpio_ite_data_3,
		    &gpio_ite_cfg_3,
		    POST_KERNEL,
