...
static int i2c_it8xxx2_recover_bus(const struct device *dev)
{
	...
	scl_pin = IT8XXX2_DT_PINMUX_PIN(config->pcfg->states->pins[SCL].pinmux);
	sda_pin = IT8XXX2_DT_PINMUX_PIN(config->pcfg->states->pins[SDA].pinmux);

	/* Set SCL of I2C as GPIO pin */
	gpio_pin_configure(config->gpio_dev, scl_pin, GPIO_OUTPUT);
	/* Set SDA of I2C as GPIO pin */
	gpio_pin_configure(config->gpio_dev, sda_pin, GPIO_OUTPUT);
	...
	gpio_pin_set(config->gpio_dev, scl_pin, 1);
	gpio_pin_set(config->gpio_dev, sda_pin, 1);
	k_msleep(1);

	/* Start condition */
	gpio_pin_set(config->gpio_dev, sda_pin, 0);
	k_msleep(1);
	gpio_pin_set(config->gpio_dev, scl_pin, 0);
	k_msleep(1);
