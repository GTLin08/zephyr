				 ...
				 const struct adc_channel_cfg *channel_cfg)
{
	...
	if (channel_cfg->channel_id < CHIP_ADC_CH4) {
		 ...
		 */
		*adc_ctrl_regs[channel_cfg->channel_id].adc_ctrl = 0xa0 +
		 ...
		 * bit7 : W/C data valid flag
		 */
		*adc_ctrl_regs[channel_cfg->channel_id].adc_ctrl = 0xb0;
	}
			      ...
			      const struct adc_sequence *sequence)
{
	...
	if (!sequence->channels ||
		sequence->channels & ~BIT_MASK(CHIP_ADC_COUNT)) {
...
static void adc_it8xxx2_get_sample(const struct device *dev)
{
	...
	while (bit != 0) {
		...
		if (adc_data_valid(idx)) {
			...
			*data->buffer = ((*adc_ctrl_regs[idx].adc_datm << 8) +
						*adc_ctrl_regs[idx].adc_datl) *
						data->ctx.sequence.resolution;
