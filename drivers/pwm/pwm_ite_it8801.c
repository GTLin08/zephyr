...
static void pwm_enable(const struct device *dev, int enabled)
{
	...
	struct pwm_it8801_data *data = dev->data;
	int ret;

	if (enabled) {
		ret = i2c_reg_update_byte_dt(data->i2c_dev, config->reg_mcr,
...
static int pwm_it8801_get_cycles_per_sec(const struct device *dev,
...
static int pwm_it8801_set_cycles(const struct device *dev,
				  ...
				  uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	if (flags & PWM_IT8XXX2_PUSH_PULL) {
		mask = it8801_pwm_gpio_map[ch].pushpull_en;

		ret = i2c_reg_update_byte_dt(data->i2c_dev, IT8801_REG_PWMODDSR,
					     mask, mask);
		if (ret != 0) {
	...
	ret = i2c_reg_write_byte_dt(data->i2c_dev, config->reg_dcr, duty);
	if (ret != 0) {
...
static int pwm_it8801_init(const struct device *dev)
{
	...
	for (int i = 0; i < MFDCTRL_ALT_COUNT; i++) {
		...
		ret = mfd_it8801_configure_pins(config->altctrl[i].gpiocr,
...
static const struct it8801_mfd_pwm_altctrl_cfg                                 \
		it8801_pwm_altctrl##inst[IT8801_DT_INST_MFCCTRL_LEN(inst)] =   \
	...
	DEVICE_DT_INST_DEFINE(inst,                                            \
			      &pwm_it8801_init,                                \
			      NULL,                                            \
			      &pwm_it8801_data_##inst,                         \
			      &pwm_it8801_cfg_##inst,                          \
			      POST_KERNEL,                                     \
			      CONFIG_PWM_IT8801_INIT_PRIORITY,                 \
			      &pwm_it8801_api);
