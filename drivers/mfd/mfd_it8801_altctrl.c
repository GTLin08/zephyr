...
static const struct i2c_dt_spec i2c_dev = I2C_DT_SPEC_GET(DT_INST(0, ite_it8801_mfd));
...
int mfd_it8801_configure_pins(const struct device *dev, uint8_t pin, uint8_t func)
{
	...
	ret = i2c_reg_update_byte_dt(&i2c_dev, reg_gpiocr,
				     GENMASK(7, 6), alt_val << 6);
	if (ret != 0) {
		LOG_ERR("Failed to read gpiocr (ret %d)", ret);
		return ret;
	}
...
#define MFD_IT8801_ALTCTRL_INIT(inst)                                          \
	...
	DEVICE_DT_INST_DEFINE(inst,                                            \
			      ...
			      NULL,                                            \
			      NULL,                                            \
			      &it8801_mfd_alt_cfg_##inst,                      \
			      POST_KERNEL,                                     \
			      CONFIG_MFD_IT8801_ALTCTRL_INIT_PRIORITY,         \
			      NULL);
